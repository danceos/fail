...
bool BasicPruner::prune_all() {
	...
	ss << "INSERT INTO fspgroup (variant_id, instr2, data_address, data_mask, fspmethod_id, pilot_id) "
	   ...
	   << "            AND t.data_address = p.data_address AND t.mask = p.data_mask "
	...
	rows = db->affected_rows();
	ss << "INSERT INTO fspgroup (variant_id, instr2, data_address, data_mask, fspmethod_id, pilot_id) "
		"SELECT STRAIGHT_JOIN t.variant_id, t.instr2, t.data_address, t.mask, p.fspmethod_id, p.id "
		"FROM fsppilot p "
		"JOIN trace t "
