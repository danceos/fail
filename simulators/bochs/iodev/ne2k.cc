...
#if BX_USE_NE2K_SMF
// theNE2kDevice may only be a poinyer in the static case there
...
int libne2k_LTX_plugin_init(plugin_t *plugin, plugintype_t type, int argc, char *argv[], const int cardNo)
{
#if BX_USE_NE2K_SMF
  ...
  BX_REGISTER_DEVICE_DEVMODEL(plugin, type, theNE2kDevice, BX_PLUGIN_NE2K);
   ...
   int nlen = strlen(BX_PLUGIN_NE2K)+2;
   char *pluginName;
   pluginName = (char*)malloc(nlen);
   memset(pluginName, 0, nlen);
   
   // snprintf may only print nlen-1 chars, cause the last one will be set to \0
   strncat(pluginName, BX_PLUGIN_NE2K, nlen-1);
   char nr[2]; 
   snprintf(nr, 2, "%d", cardNo);
   strncat(pluginName, nr, nlen);
...
void bx_ne2k_c::init(void)
{
  ...
  char configname[32];
  snprintf(configname,  32, "network.ne2k%d", interfaceNo);
  //base = (bx_list_c*) SIM->get_param(BXPN_NE2K);
  base = (bx_list_c*) SIM->get_param(configname);
  memcpy(macaddr, SIM->get_param_string("macaddr", base)->getptr(), 6);
  BX_NE2K_THIS s.pci_enabled = 0;
  //strncpy(devname, "NE2000 NIC", 16);
 sprintf(devname, "ne2000 NIC %d", interfaceNo);
  ...
  if (!BX_NE2K_THIS s.pci_enabled) {
    ...
    snprintf(configname, 32, "NE2000 ethernet NIC %d", interfaceNo); 

    DEV_register_irq(BX_NE2K_THIS s.base_irq, configname);
