...
void bx_init_options()
{
  ...
  *pci_deps_ptr = NULL;
  ...
  bx_list_c *network = new bx_list_c(root_param, "network", "Network Configuration", 32);
  network->set_options(network->USE_TAB_WINDOW | network->SHOW_PARENT);
 bx_list_c *net_menu[4];
for(i = 0; i<4; i++){
  sprintf(name, "ne2k%d", i);
  sprintf(label, "NE2000-%d", i);
// ne2k options
  //menu = new bx_list_c(network, "ne2k", "NE2000", 7);
  net_menu[i] = new bx_list_c(network, name, label, 7);
  net_menu[i]->set_options(net_menu[i]->SHOW_PARENT);
  net_menu[i]->set_enabled(BX_SUPPORT_NE2K);
  enabled = new bx_param_bool_c(net_menu[i],
    "enabled",
    "Enable NE2K NIC emulation",
    "Enables the NE2K NIC emulation",
    0);
  enabled->set_enabled(BX_SUPPORT_NE2K);
  ioaddr = new bx_param_num_c(net_menu[i],
    "ioaddr",
    "NE2K I/O Address",
    ...
    0x240+(i*0xC0));
  ioaddr->set_base(16);
  irq = new bx_param_num_c(net_menu[i],
    "irq",
    "NE2K Interrupt",
    ...
    ne2k_irq[i]);
  irq->set_options(irq->USE_SPIN_CONTROL);
  macaddr = new bx_param_string_c(net_menu[i],
    "macaddr",
    "MAC Address",
  ...
  ethmod = new bx_param_enum_c(net_menu[i],
    "ethmod",
    "Ethernet module",
  ...
  enabled->set_dependent_list(net_menu[i]->clone());
  // pnic options
}  
...
static int parse_line_formatted(const char *context, int num_params, char *params[])
{
  ...
  // ataX-master, ataX-slave
    ...
    // first find the card number in the line: then we loop the rest
    int cardNo = 0;
    for(i=1; i<num_params; i++){
    ...
    BX_INFO(("Reading NE2L, Card %i Configuration", cardNo));
    char tmpparamname[16];
    snprintf(tmpparamname, 16, "network.ne2k%d", cardNo);
     

    base = (bx_list_c*) SIM->get_param(tmpparamname);
...
int bx_write_ne2k_options(FILE *fp, bx_list_c *base, int cardNumber)
{
  fprintf(fp, "ne2k: card=%d, enabled=%d",cardNumber,  SIM->get_param_bool("enabled", base)->get());
  if (SIM->get_param_bool("enabled", base)->get()) {
    char *ptr = SIM->get_param_string("macaddr", base)->getptr();
...
int bx_write_configuration(const char *rc, int overwrite)
{
  ...
  bx_write_ne2k_options(fp, (bx_list_c*) SIM->get_param(BXPN_NE2K0), 0);
  bx_write_ne2k_options(fp, (bx_list_c*) SIM->get_param(BXPN_NE2K1), 1);
  bx_write_ne2k_options(fp, (bx_list_c*) SIM->get_param(BXPN_NE2K2), 2);
  bx_write_ne2k_options(fp, (bx_list_c*) SIM->get_param(BXPN_NE2K3), 3);
