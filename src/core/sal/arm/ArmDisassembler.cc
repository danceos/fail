...
static fail::Logger log("ARMDisas",false);
#include <string.h>
#include <stdio.h>
// ignore deprecated conversion from string constant to char*
				      ...
				      uint32_t address, struct arm_instruction *instruction)
{
	...
	if (((opcode & 0x0ff00000) == 0x0c400000) || ((opcode & 0x0ff00000) == 0x0c400000)) {
		...
		if ((opcode & 0x0ff00000) == 0x0c400000) {
			...
			log << "Unknown instruction" << std::endl;
			return ERROR_FAIL;
		}
			  ...
			  struct arm_instruction *instruction)
{
	...
	if ((opcode & 0x01800000) == 0x00000000) {
		instruction->type = static_cast<enum arm_instruction_type>(evaluate_p_add_sub(opcode, address, cp));
		return ERROR_OK;
	}
	...
	if ((opcode & 0x018000f0) == 0x00800070) {
		instruction->type = static_cast<arm_instruction_type>(evaluate_extend(opcode, address, cp));
		return ERROR_OK;
	}
			...
			struct arm_instruction *instruction)
{
	...
	log << "ARM: should never reach this point (opcode= << " << std::hex << (unsigned) opcode << ")" << std::endl;
	return -1;
}
				 ...
				 struct arm_instruction *instruction)
{
	...
	snprintf(instruction->text, 128,
			...
			address, opcode,
			x.c_str(), y.c_str(), z.c_str(), arm_condition_strings[cond]);
...
int thumb_evaluate_opcode(uint16_t opcode, uint32_t address, struct arm_instruction *instruction)
{
	...
	log << "Thumb: should never reach this point (opcode=" << opcode << ")" << std::endl;
	return -1;
}
...
ldrxb_immediate_t2:
				sprintf(cp, "%s\tr%d, [r%d%s, #%d%s\t; %#8.8x",
						mnemonic, rt, rn, p1.c_str(),
						immed, p2.c_str(), immed);
				return ERROR_OK;
			}
			if ((op2 & 0x24) == 0x24) {
				mnemonic = "LDRB";
 ...
 */
int thumb2_opcode(uint32_t address, uint32_t opcode, struct arm_instruction *instruction)
{
	int retval;
	char *cp;
	...
	switch (opcode & 0xf8000000) {
		...
		default:
			...
			return thumb_evaluate_opcode(opcode & 0x0000FFFF, address, instruction);
	}
	...
	log << "Can't decode 32-bit Thumb2 yet (opcode=" << opcode << ")" << std::endl;
...
int arm_access_size(struct arm_instruction *instruction)
{
	if ((instruction->type == ARM_LDRB)
		...
		log << "BUG: instruction type " << instruction->type << " isn't a load/store instruction" << std::endl;
		return 0;
	}
}
